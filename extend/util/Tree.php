...
class Tree
{
    ...
    public function get_childs($myid)
    {
        if (is_array($this->arr)) {
            foreach ($this->arr as $id => $a) {
                if ($a['parentid'] == $myid) {
                    $this->catarr[$id] = $a['id'];
                    $this->get_childs($a['id']);
                }
            }
        }
        return $this->catarr ? $this->catarr : false;
    }
    ...
    public function get_tree_child($myid, $str, $sid = 0, $adds = '', $str_group = '')
    {
        $number = 1;
        $child = $this->get_child($myid); //得到子级同级数组
        if (is_array($child)) {
            foreach ($child as $id => $value) {
                $this->child .= $id;
                $this->get_tree_child($id, $str, $sid, $adds . $k . $nbsp, $str_group);
                $number++;
            }
        }
        return $this->ret;
